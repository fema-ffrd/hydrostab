...
def fft_stable(
    ...
    df_hydrograph = pd.read_csv(hydrograph_path)
    hydrograph = df_hydrograph["flow"].values
...
for f in files:
    ...
    #Fourier Transform
    is_stable, _, _, _ = fft_stable(f_path)
    ...
    if is_stable:
        ls_fourier.append("Stable")
    else:
        ls_fourier.append("Unstable")
...
df_hydrograph = pd.DataFrame({'Hydrograph Title' : files,
                             ...
                             'Fourier Transform': ls_fourier }, 
                              columns=['Hydrograph Title','First Derivative', 'Std', 'Point Change', 'Fourier Transform'])
